 ...
 * flag check, a legitimate PATH substring such as `/usr/bin` or `/tmp/x.log`
 * would be silently reinterpreted as the regex `usr/bin` / `tmp/x.log` (where
 * `.` is a wildcard), quietly widening an allow rule past what the user wrote.
 *
 ...
 * `tool: "*"` and an empty `match` short-circuits EVERY escalation of EVERY
 * tool before the model runs — i.e. it is an off switch for the whole control,
 * reachable from the settings page in three clicks. Deny rules are unaffected
 * (a blanket deny is a legitimate lockdown).
 *
...
export function isBlanketAllow(effect, tool, match) {
  return effect === "allow" && tool === "*" && match === "";
}
